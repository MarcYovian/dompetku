{{-- resources/views/livewire/fund-sources.blade.php --}}

<div x-data="{ showConfirmModal: false, deleteId: null }">
    <div class="p-4 sm:p-6 text-gray-900 dark:text-gray-100">

        {{-- Header --}}
        <h2 class="text-2xl font-semibold mb-6">Manage Fund Sources</h2>
        ...
        <form wire:submit="saveFundSource" class="mb-8 p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md">
            <h3 class="text-lg font-medium mb-4">
                {{ $editingFundSourceId ? 'Edit Fund Source' : 'Create New Fund Source' }}</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                {{-- Input Name --}}
                <div>
                    <label for="name"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300">Name</label>
                    <input type="text" id="name" wire:model="name" placeholder="e.g., Bank BCA, GoPay"
                ...
                <div>
                    <label for="balance" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Initial
                        Balance</label>
                    <input type="number" id="balance" wire:model="balance" placeholder="0"
                        class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50"
                        step="0.01">
                    @error('balance')
                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
            ...
            <div class="mt-6 flex items-center justify-end space-x-4">
                ...
                <button type="submit"
                    ...
                    {{ $editingFundSourceId ? 'Update Source' : 'Add Source' }}
                </button>
            </div>
        </form>

        <div class="bg-white dark:bg-gray-800 shadow-md rounded-xl overflow-hidden">
            <h3 class="text-lg font-medium p-6">Your Fund Sources</h3>
                ...
                <div class="px-4 pb-4 space-y-3">
                    @forelse ($fundSources as $fundSource)
                        <div class="flex justify-between items-center p-4 border dark:border-gray-700 rounded-lg">
                            <div>
                                <p class="font-medium text-gray-900 dark:text-gray-100">{{ $fundSource->name }}</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Rp
                                    {{ number_format($fundSource->balance, 0, ',', '.') }}</p>
                            </div>
                            <div class="flex items-center space-x-2">
                                <button wire:click="editFundSource({{ $fundSource->id }})"
                                ...
                                <button @click="showConfirmModal = true; deleteId = {{ $fundSource->id }}"
                    ...
                    @empty
                        <p class="text-center text-gray-500 dark:text-gray-400 py-8">No fund sources found.</p>
                    @endforelse
                </div>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse ($fundSources as $fundSource)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">Rp
                                    {{ number_format($fundSource->balance, 0, ',', '.') }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="3"
                                    class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">No fund
                                    sources found.</td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div x-show="showConfirmModal" x-transition:enter="ease-out duration-300"
            ...
            <div class="mt-6 flex justify-end space-x-4">
                <button @click="showConfirmModal = false"
                    class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 dark:focus:ring-offset-gray-800">
                    Cancel
                </button>
                <button @click="$wire.deleteFundSource(deleteId); showConfirmModal = false;"
                    class="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 dark:focus:ring-offset-gray-800">
                    Delete
                </button>
            </div>
        </div>
